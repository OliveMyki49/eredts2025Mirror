...
use App\Mail\acknowledgementNotifier;

class acknowledgementNotifierController extends Controller
{
    public function sendacknowledgementNotifier(Request $request)
    {
        ...
        try {
            Mail::to($sendTo)->send(new acknowledgementNotifier($data));


            //record log
            $this->recordAction('Acknowledgement email has been sent to ' . $sendTo);
