...
<body class="g-sidenav-show  bg-gray-200" oncontextmenu="return false;">
    ...
    @if ($doc_info->compliance_deadline != null)
        <div class="text-center my-3">
            <button class="btn btn-outline-primary btn-save-order-of-payment">Create</button>
            <a class="btn btn-outline-primary btn-add-another-oop" href="/add-another-oop/{{ $doc_info->id }}/" target="_blank">Add another order of payment</a>
        </div>

        <div class="text-center my-3 lbl_additional_order_of_payment">
            <h3 class="bg-warning rounded p-2 fw-bold">Additional Order Of Payment</h3>
        </div>
        ...
        <div class="alert alert-success alert-dismissible fade show my-2 mx-4" data-bs-dismiss="alert" role="alert">
            <strong>New order of payment form opened</strong>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
                                ...
                                <td class="px-5 pb-4" colspan="2">
                                    <div class="row m-0 justify-content-end">
                                        ...
                                        <div class="col-2 text-start border-bottom border-dark">
                                            @php
                                                ...
                                                $or = $generatedOr;
                                            @endphp
                                            <input type="text" class="border-0 fs-12 bg-transparent fw-bold text-center" id="or_no" name="or_no" oninput="this.style.width = ((this.value.length * 25) + 5) + 'px';" placeholder="" value="{{ $or }}">
                                        </div>
                                    </div>
                                    <div class="row m-0 justify-content-end">
                                        <div class="col-1 text-start fw-bold">
                                    ...
                                    <div class="row m-0 mt-2">
                                        ...
                                        <div class="col-2 border-bottom border-dark text-center">
                                            <div class="d-flex flex-row">
                                                <div>PHP </div>
                                                <div><input type="text" id="pay_amount" name="pay_amount" class="border-0 fs-12 bg-transparent" oninput="this.style.width = ((this.value.length * 25) + 5) + 'px';" placeholder="Input Amount" value=""></div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row m-0 mt-2">
                                        ...
                                        <div class="col-9 border-bottom border-dark text-center">
                                            ...
                                            {{-- <input type="text" id="payment_for" name="payment_for" class="border-0 fs-12 bg-transparent text-center" oninput="this.style.width = ((this.value.length * 15) + 5) + 'px';" placeholder="Payment for . . ." value="{{ $doc_info->req_type_full }}"> --}}
                                        </div>
                                        <div class="col-3">
                                            {{-- white space --}}
                                        </div>
        ...
        <div class="text-center my-3">
            ...
            <button class="btn btn-outline-primary btn-save-order-of-payment">Create</button>
            <a class="btn btn-outline-primary btn-add-another-oop" href="/add_another_oop/{{ $doc_info->id }}/" target="_blank">Add another order of payment</a>
        </div>
        ...
        <script>
            $(function() {
                ...
                $('.btn-save-order-of-payment').click(function(e) {
                    ...
                    if (pay_amount != '' && payment_for != '' && pay_amount_total != '' && clerk_fullname != '' && prepared_by_position != '' && approving_fullname != '' && approving_position != '') {
                        if (cost_break_down_item_count == cost_break_down_item_name_count) {
                            ...
                            $.ajax({
                                url: "{{ url('insert-additional-order-of-payment') }}",
                                method: "POST",
                                data: submitForm,
                                headers: {
                                ...
                                success: function(r) {
                                    if (r.success) {
                                        ...
                                        // clear inputs
                                        $('#pay_amount').val('');
                                        $('.pay_amount_in_text').empty().append('not a number');
                                        $('#pay_amount_total').val('');
                                        $('.grand_total_payment').empty().append('');

                                        alert('Additional order of payment has been saved')
                                    } else {
                ...
                function print_item() {
                    ...
                    $('.lbl_additional_order_of_payment').hide();
                    ...
                    $('.lbl_additional_order_of_payment').show();
