...
<script>
    $(function() {
            ...
            $('#dtCliReqRec').dataTable({
                ...
                buttons: [
                    'pageLength',
                    {
                        ...
                        title: `RECEIVED CLIENT REQUESTS INCLUDING RELEASED, ARCHIVED, AND REJECTED REQUEST BY THE USER ${new Date().toLocaleDateString()}`,
                        exportOptions: {
                            columns: ':visible'
                        }
                    },
                    {
                        ...
                        customize: function(win) {
                            ...
                            $(win.document.body).find('h1').parent().prepend('' +
                                ...
                                '</div>' +
                                '<div style="text-align: center; font-size: 15pt;">' +
                                '   <h2>RECEIVED CLIENT REQUESTS INCLUDING RELEASED,<br>ARCHIVED, AND REJECTED REQUEST BY THE USER</h2>' +
                                '   <sup>User: ' + auth_username + '</sup><br>' +
                                '</div>'
                            );
                            // $(win.document.body).find('table thead').remove();
                            // $(win.document.body).find('table').prepend($('#dtCliReqRec thead').clone());
                            $(win.document.body).find('table tbody').remove();
                            $(win.document.body).find('table').append($('#dtCliReqRec tbody').clone());
                        }
                    },
                    {
                        extend: 'copy',
                    ...
                    'colvis',
                    {
                        ...
                        text: '<input class="form-control form-control-sm border-0 dtCliReqRec_search" type="text" placeholder="Search">',
        ...
        $('label[for=tabCliReqRec]').click(function() {
