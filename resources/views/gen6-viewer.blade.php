<div class="vViewer_showOfficeViewList mb-2">
    Offices Here
</div>

{{-- region display table --}}
<table class="table table-striped table-bordered dataTable " id="dtViewer" role="grid" aria-describedby="Table"
...
<div class="modal fade" id="vViewervDocAtchRcvModal" tabindex="-1" aria-labelledby="vViewer_viewDocAtchTransitLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h1 class="modal-title fs-5 text-white" id="vViewer_viewDocAtchTransitLabel">Document Attachement:
                    <b><u><span class="vViewerDocAtchName fs-6"></span></u></b>
                </h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" style="height: 800px; overflow-y: scroll;">
                <div class="vViewer_vDocAtchArchCollection"></div>
                <embed id="vViewer_vDocAtchArchIframe" name="vViewer_vDocAtchArchIframe" width="100%" height="80%"
                    style="background:url(../assets/img/denrloadsmaller.webp) center center no-repeat;">
                <div class="vViewer_other_req_inputs"></div>
            </div>
...
<div class="modal fade" id="vViewerModal" tabindex="-1" aria-labelledby="vViewerLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h1 class="modal-title fs-5 text-white" id="vViewerLabel">DOCUMENT INFORMATION</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <div class="text-center fs-3 border border-3 vViewerheader-status">
                STATUS UNKNOWN
            </div>
            ...
            <div class="modal-body p-0">
                ...
                {{-- region accordian --}}
                <div class="accordion" id="vViewer_accordian">
                    {{-- GENERAL INFORMATION --}}
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button class="accordion-button collapsed border-secondary" type="button"
                                data-bs-toggle="collapse" data-bs-target="#vViewer_collapsible_gen_info"
                                aria-expanded="true" aria-controls="vViewer_collapsible_gen_info">
                                GENERAL INFORMATION
                            </button>
                        </h2>
                        <div id="vViewer_collapsible_gen_info" class="accordion-collapse collapse"
                            data-bs-parent="#vViewer_accordian">
                            <div class="accordion-body">
                                <div class="row vViewer-content">
                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                data-bs-target="#vViewer_collapsible_req_info" aria-expanded="false"
                                aria-controls="vViewer_collapsible_req_info">
                                DOCUMENT REMARKS AND ATTACHMENTS
                            </button>
                        </h2>
                        <div id="vViewer_collapsible_req_info" class="accordion-collapse collapse"
                            data-bs-parent="#vViewer_accordian">
                            <div class="accordion-body">
                                <div class="row vViewer_doc_inpts">
                    ...
                    {{-- region form inputs --}}
                    <form class="p-0" id="addvViewerForm">
                        ...
                        <div>
                            <div class="row">
                                <div class="col-md-3 text-center">
                                    <span class="fs-6 vViewer_ttlDays_ttl_tat btn btn-primary"></span>
                                </div>
                                <div class="col-md-3 text-center">
                                    <span class="fs-6 vViewer_ttlHours_ttl_tat btn btn-primary"></span>
                                </div>
                                <div class="col-md-3 text-center">
                                    <span class="fs-6 vViewer_ttlMinutes_ttl_tat btn btn-primary"></span>
                                </div>
                                <div class="col-md-3 text-center">
                                    <span class="fs-6 vViewer_ttlSeconds_ttl_tat btn btn-primary"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row m-3 text-center">
                    <button class="btn btn-outline-primary border-0 vViewerscrollToTop">Return to top</button>
                    <script>
                        $(document).ready(function () {
                            $('.vViewerscrollToTop').click(function () {
                                $('#vViewerModal').scrollTop(0);
                            });
                        });
                    </script>
                </div>
...
@if (request()->has('active_tab'))
    ...
    @endphp
    <input type="hidden" id="dtViewer_loader" @if ($active_tab == 'tabViewSpecific') value="load" @endif>
@endif
...
<script>
    $(function () {
        // region populate table and other functions
        function dtViewer_func() {
            $.ajax({
                url: "/get-clientReqViewerOffices",
                method: "GET",
                headers: {
                ...
                success: function (r) {
                    if (r.success) {
                        //show the list of office that can be viewed by the user
                        $('.vViewer_showOfficeViewList').empty();
                        r.viewerspecific_office.forEach(office => {
                            $('.vViewer_showOfficeViewList').append(`
                                <span class="btn btn-sm btn-primary" style="cursor: pointer">
                                    ${office.full_office_name} (${office.doc_counts})
                                </span>
                            `);
                        });
            ...
            $('#dtViewer tfoot th').each(function () {
                let title = $(this).text();
            ...
            $('#dtViewer').dataTable({
                ...
                columns: [
                    ...
                    /* column 1 : doc_no */
                    {
                        ...
                        render: function (data, typw, row) {
                            return '<a' +
                                ...
                                '   class="btn-fetchdocInfo_vViewer dropdown-item" ' +
                                '   data-bs-toggle="modal" ' +
                                '   data-bs-target="#vViewerModal"' +
                    ...
                    /* column 3 : Origin Office */
                    {
                        data: 'full_office_name',
                        render: function (data, type, row) {
                            let full_office_name = row['full_office_name'];

                            if (full_office_name !== null) {
                                return full_office_name;
                            } else {
                                return '<div class="text-secondary">NONE</div>';
                            }
                        }
                    },
                    /* column 4 : application_type_id */
                    {
                        data: 'app_type',
                        render: function (data, type, row) {
                    ...
                    /* column 6 : document attachments */
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            req_attachments.forEach(dt => {
                                disp += '' +
                                    '<span ' +
                                    '   class="btnvViewer-viewDocAtch mb-1" ' +
                                    '   style="cursor: pointer;" ' +
                                    '   onMouseOut="this.style.color=`#000`" ' +
                                    '   onMouseOver="this.style.color=`#0d6efd`" ' +
                                    '   data-bs-toggle="modal" ' +
                                    '   data-bs-target="#vViewervDocAtchRcvModal"' +
                                    '   data-file_type="doc" ' +
                                    '   data-doc_id="' + row['id'] + '" ' +
                                    '   data-doc_uuid="' + row['uuid'] + '" ' +
                            ...
                            let req_atchs_collapsible = '' +
                                ...
                                '    <a class="fs-6" data-bs-toggle="collapse" href="#doc_clps_atch_btnvViewer' + row['id'] + '" role="button" aria-expanded="false" aria-controls="doc_clps_atch_btnvViewer' + row['id'] + '">' +
                                '        SHOW FILES' +
                                '    </a>' +
                                '</p>' +
                                '<div class="collapse" id="doc_clps_atch_btnvViewer' + row['id'] + '">' +
                            ...
                            return '' +
                                ...
                                '   <div class="col">' +
                                '       ' + req_atchs_collapsible +
                                '       <input type="hidden" id="vViewerdoc_fileCollection' + row['id'] + '" value="' + file_names + '">' +
                                '   </div>' +
                                '</div>' +
                                '';
                        }
                    },
                    /* column 7 : action Attachments */
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            let req_atchs_collapsible = '' +
                                '<p class="d-inline-flex gap-1">' +
                                '    <a class="fs-6" data-bs-toggle="collapse" href="#act_clps_atch_btnvViewer' + row['id'] + '" role="button" aria-expanded="false" aria-controls="act_clps_atch_btnvViewer' + row['id'] + '">' +
                                '        SHOW FILES' +
                                '    </a>' +
                                '</p>' +
                                '<div class="collapse" id="act_clps_atch_btnvViewer' + row['id'] + '">' +
                                '    <div class="card card-body">' +
                                '       ' + disp +
                                '    </div>' +
                                '</div>' +
                            ...
                            return '' +
                                ...
                                '       ' + attachment_remarks +
                                '       ' + req_atchs_collapsible +
                                '       <input type="hidden" id="vVieweract_fileCollection' + row['id'] + '" value="' + file_names + '">' +
                                '   </div>' +
                                '</div>' +
                                '';
                        }
                    },
                    /* column 8 : compliance_deadline */
                    {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        customize: function (win) {
                            ...
                            // $(win.document.body).find('table').prepend($('#dtViewer thead').clone());
                            $(win.document.body).find('table tbody').remove();
                            $(win.document.body).find('table').append($('#dtViewer tbody').clone());
                        }
                    },
                    {
                        extend: 'copy',
                    ...
                    'colvis',
                    {
                        ...
                        text: '<input class="form-control form-control-sm border-0 dtViewer_search" type="text" placeholder="Search">',
                        action: function (e, dt, node, config) {
                            let searchValue = $(node).find('.dtViewer_search').val();
        ...
        let tabViewSpecific = false;
        $('label[for=tabViewSpecific]').click(function () {
            if (tabViewSpecific == false) {
                dtViewer_func();
                tabViewSpecific = true;
            }
        });

        if ($('#dtViewer_loader').val() == 'load') {
            if (tabViewSpecific == false) {
                dtViewer_func();
                tabViewSpecific = true;
            }
        ...
        $('#dtViewer_wrapper .btn-group button').removeClass('btn-secondary').addClass('btn-outline-success');
    });
</script>
{{-- endregion populate table --}}
...
<script>
    $(function () {
        ...
        $('#dtViewer').on('click', '.btnvViewer-viewDocAtch', function () {
            ...
            if (file_type == 'doc') {
                ...
                let fileCollection = $('#dtViewer #vViewerdoc_fileCollection' + id).val();
                ...
                let content2 = $('#vViewer_vDocAtchArchIframe').attr('src');
                if (content1 != content2) {
                    $('.vViewerDocAtchName').empty().append(file_name);
                    $('#vViewer_vDocAtchArchIframe').attr('src', iframeHref + '#toolbar=0');
                }

                // get other requestee input
                $('.vViewer_other_req_inputs').empty()
                $.ajax({
                    ...
                    success: function (r) {
                        if (r.success) {
                            $('.vViewer_other_req_inputs').append('<hr><h6>Other Requestee Input:</h6><br>');

                            if (r.req_inputs.length >= 1) {
                                r.req_inputs.forEach(dt => {
                                    $('.vViewer_other_req_inputs').append('' +
                ...
                let fileCollection = $('#dtViewer #vVieweract_fileCollection' + id).val();
        ...
        $('.vViewer_vDocAtchArchCollection').on('click', '.btnvViewer-view-file', function () {
        ...
        $('#dtViewer').on('click', '.btn-fetchdocInfo_vViewer', function () {
            ...
            $.ajax({
                ...
                success: function (r) {
                    if (r.success) {
                        ...
                        // region status bar
                        if (validated != null) {
                            $('.vViewerheader-status').empty().append('<i class="fa fa-check-circle" aria-hidden="true"></i> VERIFIED');
                            $('.vViewerheader-status').removeClass('border-danger text-danger').addClass('border-success text-success');
                        } else {
                            $('.vViewerheader-status').empty().append('<i class="fa fa-hourglass" aria-hidden="true"></i> NOT YET VERIFIED');
                            $('.vViewerheader-status').removeClass('border-success text-success').addClass('border-danger text-danger');
                        }
                        // endregion status bar

                        $('.vViewer-content').empty().append('' +
                        ...
                        $('#vViewerremarks').val(remarks);

                        // region document attachments and inputs
                        let filecount = 0;
                        let inptcount = 0;
                        let app_doc_form_no = 0; //appform start from 0 
                        $('.vViewer_doc_atch').empty(); // make sure to empty the list
                        $('.vViewer_doc_inpts').empty(); // make sure to empty the list
                        ...
                        $('.vViewer_ttlDays_ttl_tat').empty().text(ttlDays_ttl_tat + ' Days');
                        $('.vViewer_ttlHours_ttl_tat').empty().text(ttlHours_ttl_tat + ' Hours');
                        $('.vViewer_ttlMinutes_ttl_tat').empty().text(ttlMinutes_ttl_tat + ' Minutes');
                        $('.vViewer_ttlSeconds_ttl_tat').empty().text(ttlSeconds_ttl_tat + ' Seconds');
                        // endregion document status
                    }
                },
                error: function (err) {
        ...
        $('.btn-vViewerreencryptId').click(function () {
            let client_id = $('#vViewerreencryptId').val();
