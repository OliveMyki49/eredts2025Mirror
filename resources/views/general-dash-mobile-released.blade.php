...
<div class="modal fade" id="vDocAtchRlsdClientIdModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="vDocAtchRlsdClientIdLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #497ac1">
                <h1 class="modal-title fs-5 text-white" id="vDocAtchRlsdClientIdLabel">Client ID View</h1>
                <button type="button" class="btn-close btn-vDocAtchRlsdreencryptId" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" style="height: 800px;">
                <iframe id="vDocAtchRlsdClientIdIframe" name="viewClientIdIframe" href="" style="width: 100%; height: 100%;"></iframe>
            </div>
            <div class="modal-footer">
                <input type="hidden" id="vDocAtchRlsdreencryptId">
                <button type="button" class="btn btn-secondary btn-vDocAtchRlsdreencryptId" data-bs-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(function() {
                        ...
                        render: function(data, type, row) {
                            ...
                            if (releasing_route != null) {
                                ...
                                releasing_route.forEach(dt => {
                                    received_status += dt.received_id != null ? '<div class="text-success fw-bold">(Received)</div>' : '<div class="text-danger fw-bold">(In-Transit)</div>';
                                    receiving_office += dt.send_to_full_office_name;


                                    disp_received_status = '<div class="text-center border text-secondary"> ' + receiving_office + '<br>' + received_status + '</div>';
                                });
                            }
                    ...
                    /* client_name */
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            let dropdownMenu = '' +
                                ...
                                '               <button ' +
                                '                  type="button" ' +
                                '                  class="btnvDIRlsd-viewClientId btn btn-outline-primary btn-sm" ' +
                                '                  data-bs-toggle="modal" ' +
                                '                  data-bs-target="#vDocAtchRlsdClientIdModal"' +
        ...
        $('button.btn_collapse_tabReleased').click(function() {
            if (tabReleased == false) {
                dTReleased_func();
                tabReleased = true;
            }

            $('.currently_viewing').empty().append('Released');
        });
...
<script>
    $(function() {
            ...
            $.ajax({
                ...
                success: function(r) {
                    if (r.success) {
                        ...
                        $('.vDIRlsd_ofp_signed_link').empty(); //remove link viewable for uploaded oop or receipt
                        $('.vDIRlsd_cli_rcpt_link').empty(); //remove link viewable for uploaded oop or receipt
                        $('.vDIRlsd-ofp-and-rcpt-uploads').empty(); //remove link viewable for uploaded oop or receipt
                        if (order_of_payment != null) {

                            // region get order of payment uploaded document links
                            if (order_of_payment.order_of_payment != null) {
                                $('.vDIRlsd_ofp_signed_link').empty().append('<a class="link-primary fs-6" href="' + order_of_payment.order_of_payment + '" target="_blank"><i class="fa fa-file-o" aria-hidden="true"></i> Order of payment</a>');
                            }

                            if (order_of_payment.payment_receipt != null) {
                                $('.vDIRlsd_cli_rcpt_link').empty().append('<a class="link-primary fs-6" href="' + order_of_payment.payment_receipt + '" target="_blank"><i class="fa fa-file-o" aria-hidden="true"></i> Receipt</a>');
                            }

                            if (order_of_payment.order_of_payment == null && order_of_payment.payment_receipt == null) {
                                $('.vDIRlsd-ofp-and-rcpt-uploads').empty().append('<div class="text-center text-danger fs-6">- - - No uploaded documents - - -</div>');
                            }
                            // endregion get order of payment uploaded document links

                            //perbill
                            $('.vDIRlsd_per_bill_no').empty().append(order_of_payment.per_bill_no);
                            $('.vDIRlsd_per_bill_no_dated').empty().append(order_of_payment.per_bill_no_dated);

                            //cost
                            if (cost_breakdown.length != 0) {
                                $('.vDIRlsd_prev_cost_breakdown').empty().append('' +
                                ...
                                cost_breakdown.forEach(dt => {
                                    $('.vDIRlsd_prev_cost_breakdown').append('' +
                                ...
                                if (total_of_cost_break_down == order_of_payment.pay_amount) {
                                    ...
                                    //show total cost
                                    $('.vDIRlsd_prev_ttl_cost').empty().append('' +
                            ...
                            $('.vDIRlsd_prev_cost_breakdown').empty();

                            //reset bill no
                            $('.vDIRlsd_per_bill_no').empty()
                            $('.vDIRlsd_per_bill_no_dated').empty()
        ...
        $('.btn-vDocAtchRlsdreencryptId').click(function() {
            let client_id = $('#vDocAtchRlsdreencryptId').val();
